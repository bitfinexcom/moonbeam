...
class RoutesPriv {
  ...
  _signupAffiliate (req, data, cb) {
    ...
    const { getGrenacheReq } = this.plugins.grenacheService
    const request = getGrenacheReq('signup', args, 'rest:core:affiliate')
    request(req, {
      json: (data) => {
        if (!data.success) {
  ...
  _getAuthToken (req, user, cb) {
    const { getGrenacheReqWithIp } = this.plugins.grenacheService
