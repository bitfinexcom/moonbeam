...
const { notifySignup, getCompetitionResultsVol } = require('./gr-client')
...
class RoutesPriv {
  ...
  _writeCompResEntries (user, res) {
    notifySignup({ user }, (err, result) => {
      if (err) {
        log(err)
  ...
  _getVolLeaderboard (res) {
    getCompetitionResultsVol((err, result) => {
      if (err) {
        log(err)
        return sendGenericError(res)
      }
      ...
      if (result.status === 'ok') {
        return res.status(200).json({ leaderboard: result.leaderboard })
      }
  ...
  onCompetitionSignUpHttpRequest (req, res) {
    this._ensureUserEligible(req, res, data => {
      const meta = data.verify.meta
      const user = meta.actions[0].authorization[0].actor

      this._writeCompResEntries(user, res)
    })
  }

  onCompetitionVolLbHttpRequest (req, res) {
    this._ensureUserEligible(req, res, () => this._getVolLeaderboard(res))
  }
