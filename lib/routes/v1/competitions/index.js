...
router.get('/active', asyncMw(async (req, res) => {
  ...
  if (!result.success) throw getCompetitionError(result.message)
  if (!result.data || !result.data.competition) throw new NotFoundError('No active competition found', { code: 'ERR_NOT_FOUND' })
...
router.get('/:id', getSchemaValidationMw(competitionId, 'params'), asyncMw(async (req, res) => {
  ...
  if (!result.success) throw getCompetitionError(result.message)
  if (!result.data || !result.data.competition) throw new NotFoundError('No competition found', { code: 'ERR_NOT_FOUND' })
...
router.post('/:id/signup/status', getSchemaValidationMw(competitionId, 'params'), getSchemaValidationMw(auth), checkAuth,
  asyncMw(async (req, res) => {
    ...
    if (!result.success) throw getCompetitionError(result.message)
    if (!result.data || !result.data.subscription) throw new NotFoundError('No competition found', { code: 'ERR_NOT_FOUND' })
