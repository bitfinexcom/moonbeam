...
class Moonbeam {
  ...
  listen (cb = () => {}) {
    ...
    this.startPlugins(this._plugins, (err) => {
      ...
      this.qrRoutes = new RoutesQR(this.conf, this.plugins)
      this.setupRoutes(
        this.pubRoutes,
        this.privRoutes,
        this.qrRoutes
  ...
  setupRoutes (pubRoutes, privRoutes, qrRoutes) {
    const { cors } = this.conf
    const cmw = _cors(cors)

    app.options('*', cmw)

    app.post('/login', cmw, qrRoutes.loginHttpRequest.bind(qrRoutes))
    app.post('/verify-login', cmw, qrRoutes.verifyLoginHttpRequest.bind(qrRoutes))
