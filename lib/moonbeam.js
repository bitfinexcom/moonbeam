...
const schema = {
  ...
  meta: Joi.object({
    ...
    expiration: Joi.string().length(19).required()
  }).max(20).unknown(true)
}
...
class Moonbeam {
  constructor (conf, plugins) {
    ...
    const fconf = {
      httpEndpoint: fc.httpEndpoint,
      keyProvider: fc.keyProvider,
      chainId: fc.chainId,
      verbose: false
    }

    this.faucetEos = Eos(fconf)
  }
  ...
  onFaucetRequest (req, res) {
    ...
    this.faucetEos
      ...
      .then((contract) => {
        return contract.send(user, opts)
      })
      .then((tx) => {
