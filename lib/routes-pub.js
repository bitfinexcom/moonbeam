...
const { getCompetition } = require('./gr-client')
...
class RoutesPub {
  constructor (conf, plugins) {
    ...
    this.onTimeHttpRequest = this.onTimeHttpRequest.bind(this)
    this.onTosHttpRequest = this.onTosHttpRequest.bind(this)
    this.onPublicTradesHttpRequest = this.onPublicTradesHttpRequest.bind(this)
    this.onCandlesHistHttpRequest = this.onCandlesHistHttpRequest.bind(this)
    this.onCandlesLastHttpRequest = this.onCandlesLastHttpRequest.bind(this)
    this.onCompetitionHttpRequest = this.onCompetitionHttpRequest.bind(this)
  ...
  onCompetitionHttpRequest (req, res) {
    getCompetition((err, result) => {
      ...
      if (result.status === 'ok') {
        return res.status(200).json({ competition: result })
      }

      res.status(result.hCode || 500).json({ error: result.error || 'ERR_GENERIC' })
    })
  }
}
