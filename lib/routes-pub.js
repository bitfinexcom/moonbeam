...
class RoutesPub {
  ...
  onTickerHttpRequest (req, res) {
    const { tickerSymbols } = this.conf
    const { tickerRequest } = this.plugins.grenacheService
    ...
    tickerRequest({ action: 'getSymbolTicker', args: [{ symbol }] }, result => {
      if (!result.success) {
  ...
  onTickersHttpRequest (req, res) {
    ...
    if (symbols === 'ALL') {
      ...
      symbols = _(symbols).split(',').uniq().value()
      const unknownSymbols = _.difference(symbols, tickerSymbols)
      if (unknownSymbols.length) {
        return res.status(400).json({ error: 'ERR_BAD_REQUEST' })
      }
    }

    tickerRequest({ action: 'getSymbolsTickers', args: [{ symbols }] }, result => {
      if (!result.success) {
        return sendGenericError(res)
      }
