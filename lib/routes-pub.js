  ...
  async onRegisterHttpRequest (req, res) {
    try {
      ...
      */
      ...
      //  verify input json
      // verify captcha
      const { hcaptcha, currentTos, cosign } = this.conf
      ...
      const txUintArray = Serialize.hexToUint8Array(tx.serializedTransaction)
      const { actions } = this.api.deserializeTransaction(txUintArray)
      if (actions.length !== 1) {
      ...
      // TODO: verify data => obtain contract data and deserialize
      // const actionData = Serialize.deserializeActionData('cosign contract data', cosign.account, 'reguser', action.data, new TextEncoder(), new TextDecoder())
      // const actionDataKeys = Object.keys(actionData)
      // const fieldNames = ['account', 'pubkey', 'tos']
      // if (actionDataKeys.length !== fieldNames.length) {
      //   return res.status(400).json({ error: 'ERR_TX_INVALID', _info: 'Wrong action data' })
      // }
      // const dataKeysCorrect = actionDataKeys.every(key => fieldNames.includes(key)) && action.authorization[1].actor === actionData.account
      // if (!dataKeysCorrect) {
      //   return res.status(400).json({ error: 'ERR_TX_INVALID', _info: 'Wrong action data' })
      // }
      // if (actionData.tos !== currentTos) {
      //   return res.status(400).json({ error: 'ERR_TX_INVALID', _info: 'Wrong TOS version' })
      // }
      // ======================================
